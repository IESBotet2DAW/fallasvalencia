...
try {
	...
	if (!$cp->getAcceso($_SESSION['rol'], $arrPermisos))
		throw new PermisosException(2, '[2] Acceso denegado');
	
	// se recoge la acción pasada por el usuario
	$strAccion = (isset($_POST['accion'])) ? $_POST['accion'] : 'default';

	// dependiendo de la acción de usuario se solicita una acción u otra al controlador
	switch ($strAccion) {

		// se ha solicitado la acción insertar
		case 'insertar':
			$cp->mostrarFallas();
			$cp->insertarMonumento();
			break;

		// si no hay acción se elige la acción por defecto
		default:
			// se solicitan las fallas, donde se añade la también la vista
			$cp->mostrarFallas();
			break;
	}
